...
namespace Scheduleless.Endpoints
{
	...
	public class FeaturedShiftEndpoint
	{
		...
		public async Task<ApiResponse<FutureShift>> FeaturedAsync<FutureShift>(RequestCachePolicy cachePolicy = RequestCachePolicy.RefreshIfNeeded)
		{
			using (var client = new AuthenticatedApiRequest())
			{
				Debug.WriteLine("In Featured Async Endpoint");
				return await client.GetAsync<FutureShift>(_baseRelativeUrl, responseMapperKey: "featured_shift", cachePolicy: cachePolicy);
			}
		}
