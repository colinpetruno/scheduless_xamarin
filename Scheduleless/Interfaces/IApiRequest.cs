...
namespace Scheduleless.Interfaces
{
	public interface IApiRequest : IDisposable
	{
		...
		Task<ApiResponse<T>> GetAsync<T>(string relativeUrl, Dictionary<string, object> parameters = null, string responseMapperKey = null, RequestCachePolicy cachePolicy = RequestCachePolicy.RefreshIfNeeded);
		...
		/// <typeparam name="T">The type of object returned from the request.</typeparam>
		Task<ApiResponse<T>> PostAsync<T>(string relativeUrl, Dictionary<string, object> parameters = null, string responseMapperKey = null, bool forceLogoutOnUnauthorized = true, RequestCachePolicy cachePolicy = RequestCachePolicy.RefreshIfNeeded);
